...
it to audio with espeak-ng, and rebuilds the podcast RSS feed that
...
def text_to_speech(script_text, wav_path):
    subprocess.run(
        ["espeak-ng", "-s", "165", "-v", "en-us", "-w", wav_path, script_text],
        check=True,
    )
...
def main():
    ...
    wav_path = f"{EPISODES_DIR}/{date_str}.wav"
    mp3_filename = f"{date_str}.mp3"
    mp3_path = f"{EPISODES_DIR}/{mp3_filename}"

    text_to_speech(script_text, wav_path)
